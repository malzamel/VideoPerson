...
class PersonDetectionResult:
    ...
    detailed_frames: int
    detailed_person_boxes: int
    detailed_face_boxes: int
    matched_person_face_pairs: int
...
class DetectionConfig:
    ...
    fast_scan_width_override: int | None = None
    detailed_scan_width_override: int | None = None
    yolo_conf: float = 0.25
    face_conf_candidate: float = 0.35
    min_face_width_candidate: int = 20
    min_face_height_candidate: int = 20
    min_sharpness_candidate: float = 15.0
    min_quality_candidate: float = 0.25
    face_conf_final: float = 0.50
    min_face_width_final: int = 32
    min_face_height_final: int = 32
    min_sharpness_final: float = 25.0
    min_brightness_candidate: float = 25.0
    max_brightness_candidate: float = 235.0
    min_brightness_final: float = 30.0
    max_brightness_final: float = 230.0
    min_quality_score: float = 0.42
    min_face_inside_frame_ratio: float = 0.80
    face_detector_mode: Literal["short_range", "full_range"] = "full_range"
    face_detector_min_confidence: float = 0.35
    ...
    def fast_scan_width(self) -> int:
        if self.fast_scan_width_override is not None:
            return self.fast_scan_width_override
        if self.mode == "ultra_fast":
            return 720
        if self.mode == "accurate":
            return 1280
    ...
    def detailed_width(self) -> int:
        if self.detailed_scan_width_override is not None:
            return min(max(self.detailed_scan_width_override, 960), 3840)
        if self.mode == "ultra_fast":
            return 960
...
class PersonDetector:
    def __init__(self) -> None:
        ...
        self._face_mode: str | None = None
        self._face_min_conf: float | None = None
        self._face_detector = None
        self._ensure_face_detector(DetectionConfig())

    def _ensure_face_detector(self, cfg: DetectionConfig) -> None:
        desired_mode = cfg.face_detector_mode
        desired_conf = float(cfg.face_detector_min_confidence)
        if self._face_detector is not None and self._face_mode == desired_mode and self._face_min_conf == desired_conf:
            return
        model_selection = 1 if desired_mode == "full_range" else 0
        self._face_detector = mp.solutions.face_detection.FaceDetection(
            model_selection=model_selection,
            min_detection_confidence=desired_conf,
        )
        self._face_mode = desired_mode
        self._face_min_conf = desired_conf
    ...
    def _quality_score(
        ...
        sharpness_norm = min(sharpness / 160.0, 1.0)
        brightness_norm = max(0.0, 1.0 - abs(brightness - 128.0) / 128.0)
        face_size_norm = min(min(float(face_width_px), float(face_height_px)) / 80.0, 1.0)
    ...
    def _match_faces_to_persons(
    ...
    ) -> list[tuple[tuple[int, int, int, int, float], tuple[int, int, int, int, float]]]:
        matches: list[tuple[tuple[int, int, int, int, float], tuple[int, int, int, int, float]]] = []
        for person in persons:
            ...
            for face in faces:
                ...
                overlap_ratio = self._intersection_area(face_box, person_box) / face_area
                if center_inside or overlap_ratio >= 0.70:
    ...
    def detect_people_and_snapshot(
        ...
        self._ensure_face_detector(cfg)
        ...
        sampled_frames = 0
        fast_pass_person_frames = 0
        fast_pass_person_boxes = 0
        ...
        for ts, frame in sample_frames_ffmpeg(
            ...
            if persons:
                fast_pass_person_frames += 1
                fast_pass_person_boxes += len(persons)
        ...
        rejected_debug_saved = 0
        detailed_frames = 0
        detailed_person_boxes = 0
        detailed_face_boxes = 0
        matched_person_face_pairs = 0
        rejected_by_reason: dict[str, int] = {}
        rejected_preview_path: Path | None = None
        rejected_preview_dir = ((debug_dir / "preview_rejected") if debug_dir is not None else (output_dir.parent / "debug" / "preview_rejected"))
        ...
        def reject_with_debug(
            ...
            nonlocal rejected_candidates, rejected_preview_path, rejected_debug_saved
            rejected_candidates += 1
            rejected_by_reason[reason] = rejected_by_reason.get(reason, 0) + 1
            if rejected_preview_path is None:
                preview_path = rejected_preview_dir / f"{video_path.stem}_{int(ts * 1000)}_rejected_preview.jpg"
                self._save_overlay(frame, preview_path, reason, person_box, face_box)
                rejected_preview_path = preview_path
            if not cfg.debug_mode or debug_dir is None:
                return
            if cfg.review_mode and rejected_debug_saved >= 20:
                return
        ...
        for start, end in windows:
            for ts, frame in sample_frames_ffmpeg(
                ...
                detailed_person_boxes += len(persons)
                if not persons:
                    reject_with_debug("person_no_face", ts, frame, None, None)
                    continue
                faces_full = self._detect_faces(frame)
                detailed_face_boxes += len(faces_full)
                matches = self._match_faces_to_persons(persons, faces_full)
                if not matches:
                    # Fallback to in-person crop detection for edge cases.
                    fallback_matches: list[tuple[tuple[int, int, int, int, float], tuple[int, int, int, int, float]]] = []
                    for person in persons:
                        px1, py1, px2, py2, _ = person
                        for face in self._detect_faces_in_person(frame, (px1, py1, px2, py2)):
                            fallback_matches.append((person, face))
                    matches = fallback_matches
                matched_person_face_pairs += len(matches)
                if not matches:
                    reject_with_debug("person_no_face", ts, frame, (persons[0][0], persons[0][1], persons[0][2], persons[0][3]), None)
                    continue

                for person, face in matches:
                    ...
                    if fconf < cfg.face_conf_candidate:
                        reject_with_debug("rejected_face_conf_candidate", ts, frame, person_box, face_box_raw)
                        continue
                    if fw < cfg.min_face_width_candidate or fh < cfg.min_face_height_candidate:
                        reject_with_debug("rejected_face_too_small_candidate", ts, frame, person_box, face_box_raw)
                        continue
                    frame_box = (0, 0, frame.shape[1], frame.shape[0])
                    raw_area = self._bbox_area(face_box_raw)
                    if raw_area <= 0:
                        reject_with_debug("invalid_crop", ts, frame, person_box, face_box_raw)
                        continue
                    raw_inside_ratio = self._intersection_area(face_box_raw, frame_box) / raw_area
                    if raw_inside_ratio < cfg.min_face_inside_frame_ratio:
                        reject_with_debug("face_out_of_frame", ts, frame, person_box, face_box_raw)
                        continue
                    ...
                    if sharpness < cfg.min_sharpness_candidate:
                        reject_with_debug("rejected_blurry_candidate", ts, frame, person_box, face_box)
                        continue
                    if brightness < cfg.min_brightness_candidate or brightness > cfg.max_brightness_candidate:
                        reject_with_debug("rejected_brightness_candidate", ts, frame, person_box, face_box)
                        continue
                    if score < cfg.min_quality_candidate:
                        reject_with_debug("rejected_quality_candidate", ts, frame, person_box, face_box)
                        continue
                    scored.append(
                        _ScoredCandidate(
                            ts,
        ...
        if not scored:
            logger.info(
                "Detection summary (no candidates): sampled_frames=%s candidate_windows=%s detailed_frames=%s "
                "person_boxes=%s face_boxes=%s matched_pairs=%s accepted_faces=0 rejected=%s",
                sampled_frames,
                len(windows),
                detailed_frames,
                detailed_person_boxes,
                detailed_face_boxes,
                matched_person_face_pairs,
                rejected_by_reason,
            )
            ...
            return PersonDetectionResult(
                ...
                detailed_person_boxes=detailed_person_boxes,
                detailed_face_boxes=detailed_face_boxes,
                matched_person_face_pairs=matched_person_face_pairs,
        ...
        if not (confirmed_person and confirmed_face):
            rejected_by_reason["no_confirmed_person_face"] = rejected_by_reason.get("no_confirmed_person_face", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        best_face_w = int(best.face_bbox[2] - best.face_bbox[0]) if best.face_bbox is not None else 0
        best_face_h = int(best.face_bbox[3] - best.face_bbox[1]) if best.face_bbox is not None else 0
        if best.face_conf < cfg.face_conf_final:
            rejected_by_reason["face_confidence_low_final"] = rejected_by_reason.get("face_confidence_low_final", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        if best_face_w < cfg.min_face_width_final or best_face_h < cfg.min_face_height_final:
            rejected_by_reason["face_too_small_final"] = rejected_by_reason.get("face_too_small_final", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        if best.sharpness < cfg.min_sharpness_final:
            rejected_by_reason["rejected_blurry_final"] = rejected_by_reason.get("rejected_blurry_final", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        if best.brightness < cfg.min_brightness_final or best.brightness > cfg.max_brightness_final:
            rejected_by_reason["rejected_brightness_final"] = rejected_by_reason.get("rejected_brightness_final", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        if best.score < cfg.min_quality_score:
            rejected_by_reason["rejected_quality_final"] = rejected_by_reason.get("rejected_quality_final", 0) + 1
            return PersonDetectionResult(
                max_people=0,
                accepted_faces=0,
                rejected_candidates=rejected_candidates,
        ...
        metadata = {
            ...
            "face_width_detection_frame": best_face_w,
            "face_height_detection_frame": best_face_h,
            "face_width_original_frame": (face_bbox_original[2] - face_bbox_original[0]) if face_bbox_original else 0,
            "face_height_original_frame": (face_bbox_original[3] - face_bbox_original[1]) if face_bbox_original else 0,
            ...
            "sampled_frames": sampled_frames,
            "fast_pass_person_frames": fast_pass_person_frames,
            "fast_pass_person_boxes": fast_pass_person_boxes,
            "candidate_windows": len(windows),
            "detailed_frames": detailed_frames,
            "detailed_person_boxes": detailed_person_boxes,
            "detailed_face_boxes": detailed_face_boxes,
        ...
        logger.info(
            "Detection summary: sampled_frames=%s candidate_windows=%s detailed_frames=%s person_boxes=%s "
            "face_boxes=%s matched_pairs=%s accepted_faces=1 rejected_by_reason=%s",
            sampled_frames,
            len(windows),
            detailed_frames,
            detailed_person_boxes,
